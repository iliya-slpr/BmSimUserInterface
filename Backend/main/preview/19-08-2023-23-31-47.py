...
ENVIRONMENT = 1000  # the dimension of the environment (square) i meters that the network nodes are spread in it
##################grid deployment ############
for i17 in range(
...
NODE_RANGE = 2  # the communication range of the nodes, assumin a unit disk model
EXECUTION_TIME = 2000  # the execution time of the simulator in milliseconds
#####loging##
...
update_mobility_interval = 10  # The interval for calling the Network_updator module (in milisecond)
...
Receive_window = 1  # the Receive window parameter in friendship mechanism( ms)
lowpower_Poll_interval = 1  # the request[friend poll] interval parameter in friendship mechanism (ms)
#####################
logger = []
energy_log = []
destination = []
