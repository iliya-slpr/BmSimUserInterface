...
#######  initial settings ##############
###### deployment setting ###########
NUMBER_NODES = 12  # the number of nodes in the network
x = []  # the positions of nodes in the network
y = []

ENVIRONMENT = 60  # the dimension of the environment (square) i meters that the network nodes are spread in it
##################grid deployment ############
for i17 in range(
...
EXECUTION_TIME = 100  # the execution time of the simulator in milliseconds
#####loging##
...
BUFFER_SIZE = 1  # the size of the nodes' buffer
Show_progress_interval = EXECUTION_TIME/100  # the resolution (%) of the simulation progress
update_mobility_interval = 1  # The interval for calling the Network_updator module (in milisecond)
...
sleep_time = 10      # the sleep time in friendship mechanism (ms)
Receive_window = 10  # the Receive window parameter in friendship mechanism( ms)
...
nodes[Center_node].Heartbeat_period = 10
