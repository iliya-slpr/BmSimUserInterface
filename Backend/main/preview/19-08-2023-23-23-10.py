...
#######  initial settings ##############
###### deployment setting ###########
NUMBER_NODES = 18  # the number of nodes in the network
...
Receive_window = 200  # the Receive window parameter in friendship mechanism( ms)
...
for i_r in range(NUMBER_NODES):
    ########## node setting ###########
    nodes[i_r].SCAN_INTERVAL = 1
    nodes[i_r].SCAN_WINDOW = 1
    nodes[i_r].Relay_Retransmit_Count = 1
    nodes[i_r].Network_Transmit_Count = 1
    nodes[i_r].Rris = 1
    nodes[i_r].Ntis = 1
    nodes[i_r].Advertise_Interval = 1
    nodes[i_r].GENERATION_INTERVAL = 1
...
nodes[Center_node].Heartbeat_period = 1
