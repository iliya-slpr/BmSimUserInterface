...
#######  initial settings ##############
###### deployment setting ###########
NUMBER_NODES = 20  # the number of nodes in the network
x = []  # the positions of nodes in the network
y = []

ENVIRONMENT = 200  # the dimension of the environment (square) i meters that the network nodes are spread in it
##################grid deployment ############
for i17 in range(
...
NODE_RANGE = 100  # the communication range of the nodes, assumin a unit disk model
EXECUTION_TIME = 1000  # the execution time of the simulator in milliseconds
#####loging##
...
mobility_flag = 1  # mobility flag determines the number of mobility updates during the simulation
update_flag = 1 # update flag determines the presence of run-time adjustment
# for parameters and models during the simulation
BUFFER_SIZE = 10  # the size of the nodes' buffer
Show_progress_interval = EXECUTION_TIME/100  # the resolution (%) of the simulation progress
update_mobility_interval = 100  # The interval for calling the Network_updator module (in milisecond)
NETWORK_TTL = NUMBER_NODES  # the initial value for the network's TTL if the user
# does not want to use the heartbeat message
#######lowpower and friend #####
Receive_Delay = 100  # the Receive Delay parameter in friendship mechanismb (ms)
sleep_time = 100      # the sleep time in friendship mechanism (ms)
Receive_window = 100  # the Receive window parameter in friendship mechanism( ms)
lowpower_Poll_interval = 100  # the request[friend poll] interval parameter in friendship mechanism (ms)
#####################
logger = []
energy_log = []
destination = []
...
for i_r in range(NUMBER_NODES):
    ########## node setting ###########
    nodes[i_r].SCAN_INTERVAL = 10
    nodes[i_r].SCAN_WINDOW = 10
    nodes[i_r].Relay_Retransmit_Count = 10
    nodes[i_r].Network_Transmit_Count = 10
    nodes[i_r].Rris = 10
    nodes[i_r].Ntis = 10
    nodes[i_r].Advertise_Interval = 10
    nodes[i_r].GENERATION_INTERVAL = 10
...
nodes[Center_node].Heartbeat_period = 100
