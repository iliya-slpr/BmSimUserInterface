...
#######  initial settings ##############
###### deployment setting ###########
NUMBER_NODES = {{nodes_number}}  # the number of nodes in the network
...
ENVIRONMENT = {{environment}}  # the dimension of the environment (square) i meters that the network nodes are spread in it
##################grid deployment ############
{{^hasNode}}
for i17 in range(
    ...
    for j17 in range(int(NUMBER_NODES / math.sqrt(NUMBER_NODES))):
        {{#random_nodes}}
        ix = random.randint(1,ENVIRONMENT)
        iy = random.randint(1,ENVIRONMENT)
        x.append(ix)
        y.append(iy)
        {{/random_nodes}}
        {{^random_nodes}}
...
NODE_RANGE = {{node_range}}  # the communication range of the nodes, assumin a unit disk model
EXECUTION_TIME = {{execution_time}}  # the execution time of the simulator in milliseconds
#####loging##
...
mobility_flag = {{mobility_flag}}  # mobility flag determines the number of mobility updates during the simulation
update_flag = {{update_flag}} # update flag determines the presence of run-time adjustment
# for parameters and models during the simulation
BUFFER_SIZE = {{buffer_size}}  # the size of the nodes' buffer
Show_progress_interval = EXECUTION_TIME/100  # the resolution (%) of the simulation progress
update_mobility_interval = {{update_mobility_interval}}  # The interval for calling the Network_updator module (in milisecond)
NETWORK_TTL = NUMBER_NODES  # the initial value for the network's TTL if the user
# does not want to use the heartbeat message
#######lowpower and friend #####
Receive_Delay = {{receive_delay}}  # the Receive Delay parameter in friendship mechanismb (ms)
sleep_time = {{sleep_time}}      # the sleep time in friendship mechanism (ms)
Receive_window = {{receive_window}}  # the Receive window parameter in friendship mechanism( ms)
lowpower_Poll_interval = {{lowpower_poll_interval}}  # the request[friend poll] interval parameter in friendship mechanism (ms)
#####################
logger = []
energy_log = []
destination = []
...
for i_r in range(NUMBER_NODES):
    ########## node setting ###########
    nodes[i_r].SCAN_INTERVAL = {{scan_interval}}
    nodes[i_r].SCAN_WINDOW = {{scan_window}}
    nodes[i_r].Relay_Retransmit_Count = {{relay_retransmit_count}}
    nodes[i_r].Network_Transmit_Count = {{network_transmit_count}}
    nodes[i_r].Rris = {{rris}}
    nodes[i_r].Ntis = {{ntis}}
    nodes[i_r].Advertise_Interval = {{advertise_interval}}
    nodes[i_r].GENERATION_INTERVAL = {{generation_interval}}
...
{{#hasCenterNode}}
Center_node =  {{center_node_number}}
...
nodes[Center_node].Heartbeat_period = {{heartbeat_period}}
